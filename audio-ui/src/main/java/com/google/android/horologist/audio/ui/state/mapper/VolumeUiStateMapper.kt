...
public object VolumeUiStateMapper {
    public fun map(volumeState: VolumeState): VolumeUiState {
        val min = volumeState.min
        val max = volumeState.max
        val current = volumeState.current
        var currentPercent: Float? = null
        if (min != null && max != null && current != null) {
            currentPercent = (current - min).toFloat() / max
        }
        return VolumeUiState(
            current = currentPercent,
            isMuted = volumeState.isMuted,
            adjustable = volumeState.absolutelyAdjustable || volumeState.relativelyAdjustable
        )
    }
}
