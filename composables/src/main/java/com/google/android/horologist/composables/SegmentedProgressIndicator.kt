...
public data class ProgressIndicatorSegment(
   ...
   * Represents a segment of the track in a [SegmentedProgressIndicator].
   *
   * @param weight The proportion of the overall progress indicator that this segment should take
   *   up, as a proportion of the sum of the weights of all the other segments.
 ...
 *   represent 6 o'clock and 9 o'clock respectively. Default is -90 degrees (top of the screen)
 ...
 *   o'clock and 9 o'clock respectively. By default equal to 270 degrees.
...
public fun SegmentedProgressIndicator(
  ...
  val totalWeight =
    remember(trackSegments) { trackSegments.sumOf { it.weight.toDouble() }.toFloat() }
  ...
  // Please see
  // https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:wear/compose/compose-material/src/commonMain/kotlin/androidx/wear/compose/material/ProgressIndicator.kt?q=CircularProgressIndicator
  val stroke = with(localDensity) { Stroke(width = strokeWidth.toPx(), cap = StrokeCap.Round) }

  Canvas(modifier = modifier.fillMaxSize().progressSemantics(progress)) {
    ...
    val endDelta =
      (asin(strokeWidth.toPx() / (size.minDimension - strokeWidth.toPx())) * 180 / Math.PI)
        .toFloat()
    ...
    for (segment in trackSegments) {
      ...
      val backgroundSweep = 360f - ((startAngleWithDelta - currentEndAngle) % 360 + 360) % 360
...
private fun DrawScope.drawCircularProgressIndicator(
  ...
  val localTrackColor =
    when {
      segment.inProgressTrackColor != null &&
        progressSweep > 0 &&
        progressSweep < backgroundSweep -> segment.inProgressTrackColor
