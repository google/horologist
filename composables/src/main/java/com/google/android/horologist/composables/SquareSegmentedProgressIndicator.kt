...
internal data class Measures(
    ...
    private fun Path.lineRange(
        ...
        val start = (drawRange.start - segmentRange.start) /
            (segmentRange.endInclusive - segmentRange.start)
        val end = (drawRange.endInclusive - segmentRange.endInclusive) /
            (segmentRange.endInclusive - segmentRange.start)
...
private fun calculateSegments(
    ...
    val edgeLength = (2 * (height - 2 * cornerRadius)) +
    ...
    return buildList {
        trackSegments.forEachIndexed { segmentIndex, trackSegment ->
            ...
            val paddedRange = (
                (startWeight + startPaddingWeight) / paddedWeight
                )..(
                (startWeight + startPaddingWeight + trackSegment.weight) / paddedWeight
                )
