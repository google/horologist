 ...
 *   of each segment.
 * @param trackColor The color of the track that is drawn behind the indicator color of the track
 *   sections.
 * @param cornerRadiusDp Radius of the corners that are on the square. If this is 0.dp it is a
 *   normal square.
...
public fun SquareSegmentedProgressIndicator(
  ...
  check(progress in 0.0..1.0) { "Only progress between 0.0 and 1.0 is allowed." }
...
internal data class Measures(
  ...
  private fun Path.lineRange(
    ...
    val end =
      (drawRange.endInclusive - segmentRange.endInclusive) /
        (segmentRange.endInclusive - segmentRange.start)
  ...
  private fun Path.arcRange(
    ...
    val start = ((drawRange.start - segmentRange.start) / segmentRangePercent).coerceIn(0f, 1f)
    val end =
      ((segmentRange.endInclusive - drawRange.endInclusive) / segmentRangePercent).coerceIn(0f, 1f)
...
private fun calculateSegments(
  ...
  val edgeLength =
    (2 * (height - 2 * cornerRadius)) +
      (2 * (width - 2 * cornerRadius)) +
  ...
  return buildList {
    trackSegments.forEachIndexed { segmentIndex, trackSegment ->
      ...
      val paddedRange =
        ((startWeight + startPaddingWeight) / paddedWeight)..((startWeight +
            startPaddingWeight +
            trackSegment.weight) / paddedWeight)

      val splitSegments =
        measures.splitSegments(
