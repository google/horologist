...
sealed interface AmbientState {
   ...
   *
   * This object is used to track whether the application is currently being interacted with by the
   * user.
   */
  ...
  /**
   * Represents the state of a device, that the app is in ambient mode and not actively updating the
   * display.
   *
   * This class holds information about the ambient display properties, such as whether burn-in
   * protection is required, if the device has low bit ambient display, and the last time the
   * ambient state was updated.
   *
   ...
   *   Defaults to false.
   * @property updateTimeMillis The timestamp in milliseconds when the ambient state was last
   *   updated. Defaults to the current system time.
