...
public interface RotaryHapticHandler {
  ...
  @ExperimentalHorologistApi public fun handleScrollHaptic(scrollDelta: Float)

  /** Handles haptics when scroll with snap is used */
  @ExperimentalHorologistApi public fun handleSnapHaptic(scrollDelta: Float)
}
...
public interface RotaryHapticFeedback {
  @ExperimentalHorologistApi public fun performHapticFeedback(type: RotaryHapticsType)
}
...
public value class RotaryHapticsType(private val type: Int) {
  public companion object {
    /**
     * A scroll ticking haptic. Similar to texture haptic - performed each time when a scrollable
     * content is scrolled by a certain distance
     */
    @ExperimentalHorologistApi public val ScrollTick: RotaryHapticsType = RotaryHapticsType(1)

    /**
     * An item focus (snap) haptic. Performed when a scrollable content is snapped to a specific
     * item.
     */
    @ExperimentalHorologistApi public val ScrollItemFocus: RotaryHapticsType = RotaryHapticsType(2)

    /**
     * A limit(overscroll) haptic. Performed when a list reaches the limit (start or end) and can't
     * scroll further
     */
    @ExperimentalHorologistApi public val ScrollLimit: RotaryHapticsType = RotaryHapticsType(3)
  }
}
    ...
    .apply {
      LaunchedEffect(hapticsChannel) {
        hapticsChannel.receiveAsFlow().throttleLatest(throttleThresholdMs).collect { hapticType ->
          ...
          withContext(Dispatchers.Default) { rotaryHaptics.performHapticFeedback(hapticType) }
        }
      }
    }
}
