...
class FastScrollingTransformingLazyColumnTest(override val device: WearDevice) :
  ...
  val people = peopleString.split(",").map { Person(it.trim()) }
  val headers = people.map { Header(it.content.take(1)) }.distinctBy { it.content }
  ...
  fun BasicExample() {
      ...
      AppScaffold(
        ...
        ScreenScaffold(
          ...
          FastScrollingTransformingLazyColumn(
            ...
            modifier = Modifier.fillMaxSize().testTag(TLC),
            headers = headers,
          ) {
            items(tlcContent) { item ->
              if (item is Header) {
                Row(horizontalArrangement = Arrangement.Center) { Text(item.content) }
    ...
    composeRule.onNodeWithTag(TLC).performRotaryScrollInput {
      repeat(15) { rotateToScrollVertically(10000.0f) }
    }
