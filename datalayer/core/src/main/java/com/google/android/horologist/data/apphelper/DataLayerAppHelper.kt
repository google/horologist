...
abstract class DataLayerAppHelper(
  ...
  public suspend fun connectedNodes(): List<AppHelperNodeStatus> {
    ...
    return connectedNodes.map {
      val appInstallationStatus =
        if (allInstalledNodes.contains(it.id)) {
          val nodeType =
            when (it.id) {
  ...
  private suspend fun getSurfaceStatus(nodeId: String) =
    registry
      .protoFlow(
  ...
  protected fun connectedAndInstalledNodes(capability: String) =
    callbackFlow<Set<Node>> {
      ...
      suspend fun listenAndSendChanges() {
        ...
        awaitClose { registry.capabilityClient.removeListener(listener) }
      }
   ...
   * @throws [IllegalArgumentException] when uri for the app store is not provided and this function
   *   is called for a node that is iOS.
   */
   ...
   * node, otherwise a [timeout](AppHelperResultCode.APP_HELPER_RESULT_TIMEOUT) is expected. See
   ...
   */
  @CheckResult abstract suspend fun startCompanion(nodeId: String): AppHelperResultCode
  ...
  public suspend fun startRemoteOwnApp(nodeId: String): AppHelperResultCode {
    checkIsForegroundOrThrow()
    val request = launchRequest { ownApp = OwnAppConfig.getDefaultInstance() }
    return sendRequestWithTimeout(nodeId, LAUNCH_APP, request.toByteArray())
  }
  ...
  protected fun checkIsForegroundOrThrow() {
    ...
    val isForeground =
      runningAppProcesses.find { it.pid == Process.myPid() }?.importance == IMPORTANCE_FOREGROUND
    if (!isForeground) {
...
fun byteArrayForResultCode(resultCode: AppHelperResultCode): ByteArray {
  val response = appHelperResult { code = resultCode }
