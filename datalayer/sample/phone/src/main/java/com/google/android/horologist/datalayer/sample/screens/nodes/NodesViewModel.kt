...
constructor(private val phoneDataLayerAppHelper: PhoneDataLayerAppHelper) : ViewModel() {
  ...
  private val _uiState = MutableStateFlow<NodesScreenState>(NodesScreenState.Idle)
  ...
  fun onStartCompanionClick(nodeId: String) {
    runActionAndHandleAppHelperResult { phoneDataLayerAppHelper.startCompanion(nodeId = nodeId) }
  }
  ...
  fun onStartRemoteOwnAppClick(nodeId: String) {
    runActionAndHandleAppHelperResult { phoneDataLayerAppHelper.startRemoteOwnApp(nodeId = nodeId) }
  }

  fun onStartRemoteActivityClick(nodeId: String) {
    runActionAndHandleAppHelperResult {
      val config = activityConfig { classFullName = REMOTE_ACTIVITY_SAMPLE_CLASS_FULL_NAME }
    ...
    viewModelScope.launch {
      when (val result = action()) {
        AppHelperResultCode.APP_HELPER_RESULT_SUCCESS -> {
          ...
          _uiState.value = NodesScreenState.ActionFailed(errorCode = result.name)
        }
      }
    }
  }
