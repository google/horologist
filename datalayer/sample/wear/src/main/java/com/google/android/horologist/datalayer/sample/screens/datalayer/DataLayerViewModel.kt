...
constructor(
  ...
  init {
    viewModelScope.launch {
      counterFlow.collectLatest { newValue ->
        uiState.update { state -> updateIfNewer(state, newValue) }
      }
    }
  }
  ...
  fun addDelta(i: Int) {
    viewModelScope.launch {
      try {
        val newValue: CounterValue = counterService.increment(counterDelta { delta = i.toLong() })
        uiState.update { updateIfNewer(it, newValue) }
      } catch (e: Exception) {
