...
constructor(private val wearDataLayerAppHelper: WearDataLayerAppHelper) : ViewModel() {
  ...
  private val _uiState = MutableStateFlow<NodesActionScreenState>(NodesActionScreenState.Idle)
  ...
  private suspend fun loadNodes() {
    _uiState.value =
      NodesActionScreenState.Loaded(
        nodeList =
          wearDataLayerAppHelper.connectedNodes().map { node ->
            val type =
              when (node.appInstallationStatus) {
