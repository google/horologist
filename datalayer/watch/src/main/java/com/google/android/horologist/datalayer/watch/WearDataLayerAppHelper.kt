...
internal constructor(
  ...
  override suspend fun startCompanion(nodeId: String): AppHelperResultCode {
    ...
    val request = launchRequest { companion = companionConfig { sourceNode = localNode.id } }
   ...
   *
   * This function has some limitations on older SDK versions, please see the docs for
   * [TileService#getActiveTilesAsync](https://developer.android.com/reference/androidx/wear/tiles/TileService#getActiveTilesAsync(android.content.Context,java.util.concurrent.Executor))
   */
   ...
   * logged out. This will roll the state back to the app having been used once - if the setup had
   * previously been completed, but will have no effect if this is not the case.
   */
   ...
   * @param complicationInstanceId Passed from
   *   [ComplicationDataSourceService.onComplicationActivated]
  ...
  public suspend fun markComplicationAsDeactivated(complicationInstanceId: Int) {
    surfacesInfoDataStore.updateData { info ->
      val filtered = info.complicationsList.filterNot { it.instanceId == complicationInstanceId }
  ...
  /**
   * Compares equality of [ComplicationInfo] excluding timestamp, as when the Complication was added
   * is not relevant.
   */
