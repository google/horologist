...
class PlayerRepositoryImplNotConnectedTest(
  ...
  companion object {
    ...
    fun params() =
      listOf(
        param("initial state") {},
        param("prepare") { sut: PlayerRepositoryImpl -> sut.play() },
        param("play") { sut: PlayerRepositoryImpl -> sut.play() },
        param("seek to default index") { sut: PlayerRepositoryImpl ->
          sut.seekToDefaultPosition(1)
        },
        param("pause") { sut: PlayerRepositoryImpl -> sut.pause() },
        param("hasPreviousMedia") { sut: PlayerRepositoryImpl -> sut.hasPreviousMedia() },
        param("skipToPreviousMedia") { sut: PlayerRepositoryImpl -> sut.skipToPreviousMedia() },
        param("hasNextMedia") { sut: PlayerRepositoryImpl -> sut.hasNextMedia() },
        param("skipToNextMedia") { sut: PlayerRepositoryImpl -> sut.skipToNextMedia() },
        param("seekBack") { sut: PlayerRepositoryImpl -> sut.seekBack() },
        param("seekForward") { sut: PlayerRepositoryImpl -> sut.seekForward() },
        param("setShuffleModeEnabled") { sut: PlayerRepositoryImpl ->
          sut.setShuffleModeEnabled(true)
        },
        param("setMedia") { sut: PlayerRepositoryImpl -> sut.setMedia(getDummyMedia()) },
        param("setMediaList") { sut: PlayerRepositoryImpl ->
          sut.setMediaList(listOf(getDummyMedia()))
        },
        param("addMedia") { sut: PlayerRepositoryImpl -> sut.addMedia(getDummyMedia()) },
        param("addMedia with index") { sut: PlayerRepositoryImpl ->
          sut.addMedia(1, getDummyMedia())
        },
        param("removeMedia") { sut: PlayerRepositoryImpl -> sut.removeMedia(1) },
        param("clearMediaList") { sut: PlayerRepositoryImpl -> sut.clearMediaList() },
        param("getMediaCount") { sut: PlayerRepositoryImpl -> sut.getMediaCount() },
        param("getMediaAt") { sut: PlayerRepositoryImpl -> sut.getMediaAt(1) },
        param("getCurrentMediaIndex") { sut: PlayerRepositoryImpl -> sut.getCurrentMediaIndex() },
