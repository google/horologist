...
/**
 * Internal [EventListener] that records estimated request and response sizes to the
 * [DataRequestRepository] as well as closing [High]
 */
...
public open class NetworkLoggingEventListenerFactory(
  ...
  internal open inner class Listener(delegate: EventListener) : ForwardingEventListener(delegate) {
    ...
    override fun connectionAcquired(call: Call, connection: Connection) {
      ...
      val networkInfo =
        if (currentNetworkInfo == null) {
          ...
          network?.networkInfo
            ?: NetworkInfo.Unknown(localAddress.toString()).also { request.networkInfo = it }
        } else {
          currentNetworkInfo
        }

      logger.debugNetworkEvent(
        "HTTPS request ${request.requestType} ${networkInfo.type} $localAddress"
      )

      callDetails =
        "HTTP ${call.request().requestType} ${call.request().networkInfo?.type?.name ?: "Unknown"}"
          .also { Trace.beginAsyncSection(it, id) }
    ...
    override fun callEnd(call: Call) {
      ...
      callDetails?.let { Trace.endAsyncSection(it, id) }
