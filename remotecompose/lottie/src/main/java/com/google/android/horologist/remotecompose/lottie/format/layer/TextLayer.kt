...
internal data class TextMoreOptions(
  ...
  val alignment: List<Float>? = null,
  @SerialName("g") val grouping: Int? = null,
)

internal object TextAlignmentSerializer : KSerializer<List<Float>?> {
  override val descriptor: SerialDescriptor =
  ...
  override fun deserialize(decoder: Decoder): List<Float>? {
    ...
    return try {
      when (element) {
        is JsonArray -> element.mapNotNull { it.jsonPrimitive.floatOrNull }
        is JsonObject -> {
          val k = element["k"]
          when (k) {
            is JsonArray -> {
              if (k.firstOrNull() is JsonObject) {
                (k.firstOrNull() as? JsonObject)?.get("s")?.let { s ->
                  (s as? JsonArray)?.mapNotNull { it.jsonPrimitive.floatOrNull }
                }
              } else {
                k.mapNotNull { it.jsonPrimitive.floatOrNull }
              }
            }
            else -> null
          }
