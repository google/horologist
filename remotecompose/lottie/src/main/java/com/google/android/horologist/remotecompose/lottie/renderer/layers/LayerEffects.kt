...
internal val LocalEffectSurface = compositionLocalOf<RemoteImageBitmap?> { null }

/**
 * Each component has a separate recorder, so carry its containing bitmap across component
 * boundaries.
 */
internal fun RemoteCanvas.withContainingSurface(surface: RemoteImageBitmap?, block: () -> Unit) {
  if (surface == null) {
    block()
    return
  }
  val recorder = internalCanvas
  val targetId = surface.getIdForCreationState(recorder.creationState)
  if (recorder.currentDrawToBitmapId == targetId) {
    block()
  } else {
    recorder.drawToOffscreenBitmap(surface) { block() }
  }
}

private fun Layer.asVisible(): Layer =
  when (this) {
    is ShapeLayer -> copy(hidden = false.rb, matteTarget = 0)
    is SolidColorLayer -> copy(hidden = false.rb, matteTarget = 0)
    is ImageLayer -> copy(hidden = false.rb, matteTarget = 0)
    is TextLayer -> copy(hidden = false.rb, matteTarget = 0)
    is PrecompLayer -> copy(hidden = false.rb, matteTarget = 0)
    else -> this
  }
...
internal fun LayerEffects(
  ...
  RemoteBox(modifier = RemoteModifier.fillMaxSize()) {
    // 1. Clear offscreen buffers before children render into them
    RemoteCanvas(modifier = RemoteModifier.fillMaxSize()) {
      remoteCanvas.internalCanvas.drawToOffscreenBitmap(target, 0) {}
      if (matte != null) {
        remoteCanvas.internalCanvas.drawToOffscreenBitmap(matteBitmap, 0) {}
      }
    }
    // 2. Render matte layer (if present) and target layer into their respective offscreen surfaces
    if (matte != null) {
      CompositionLocalProvider(LocalEffectSurface provides matteBitmap) {
        Layer(
          matte.matteLayer.asVisible(),
          parentTransforms + (matte.matteLayer.index to matte.matteTransforms),
        )
      }
    }
    ...
    // 3. Apply masks and matte compositing onto target, then draw target onto containing surface
    RemoteCanvas(modifier = RemoteModifier.fillMaxSize()) {
      val canvas = remoteCanvas
      if (masks.isNotEmpty()) {
        val initial =
        ...
        for (mask in masks) {
          ...
          canvas.internalCanvas.drawToOffscreenBitmap(maskPart, if (mask.inverted) -1 else 0) {
            canvas.save()
            for (t in transforms) transform(t, null, settings, canvas)
      ...
      if (matte != null) {
        val inverted = matte.matteMode in listOf(MatteMode.InvertedAlpha, MatteMode.InvertedLuma)
        canvas.internalCanvas.drawToOffscreenBitmap(target) {
