...
private data class RoundPoint(val x: RemoteFloat, val y: RemoteFloat) {
  ...
  fun length(): RemoteFloat = sqrt(x * x + y * y)
...
internal fun roundRemoteBezier(path: RemoteBezierValue, radius: RemoteFloat): RemoteBezierValue {
  ...
  val r = max(radius, 0f.rf).rawReference()
  ...
  val positiveRadius = selectIfLe(r, 0f.rf, 0f.rf, 1f.rf)
  for (j in 0 until capacity) {
    ...
    val split = sourceActive * round * positiveRadius
    ...
    fun offsetToward(neighbor: RoundPoint): RoundPoint {
      val delta = neighbor - current
      val length = delta.length()
      val amount = selectIfLe(length, 0f.rf, 0f.rf, min(0.5f.rf, r / max(length, 0.000001f.rf)))
      return delta * amount
    }
    val aOffset = offsetToward(previous)
    val bOffset = offsetToward(next)
    val a = roundChoose(round, current + aOffset, current)
    val b = roundChoose(round, current + bOffset, current)
    output.add(
      RoundVertex(
        a,
        roundChoose(round, zero, incoming),
        roundChoose(round, aOffset * (-0.5519f).rf, zero),
      )
    )
    output.add(
      RoundVertex(
        b,
        roundChoose(round, bOffset * (-0.5519f).rf, zero),
  ...
  return LogicalBezier(
      RemoteBezierValue(
        ...
        RemoteBezierTopology(activeVertices, activeSegments, roundedCorners = true),
