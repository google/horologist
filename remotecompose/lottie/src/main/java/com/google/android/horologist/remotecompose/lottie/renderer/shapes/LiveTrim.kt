...
private const val TRIM_LENGTH_STEPS = 16

private fun RemoteFloat.shared(): RemoteFloat =
  if (constantValueOrNull != null) this else rawReference()

private fun RemoteFloat.constAdd(other: RemoteFloat): RemoteFloat {
  val a = constantValueOrNull
  val b = other.constantValueOrNull
  return if (a != null && b != null) (a + b).rf else this + other
}

private fun RemoteFloat.constSub(other: RemoteFloat): RemoteFloat {
  val a = constantValueOrNull
  val b = other.constantValueOrNull
  return if (a != null && b != null) (a - b).rf else this - other
}

private fun RemoteFloat.constMul(other: RemoteFloat): RemoteFloat {
  val a = constantValueOrNull
  val b = other.constantValueOrNull
  return if (a != null && b != null) (a * b).rf else this * other
}

private fun RemoteFloat.constSqrt(): RemoteFloat {
  val a = constantValueOrNull
  return if (a != null) mathSqrt(a).rf else sqrt(this)
}

private fun constMax(a: RemoteFloat, b: RemoteFloat): RemoteFloat {
  val av = a.constantValueOrNull
  val bv = b.constantValueOrNull
  return if (av != null && bv != null) mathMax(av, bv).rf else max(a, b)
}
...
private fun balancedSum(
  ...
  when (to - from) {
    ...
    1 -> values[from]
      ...
      balancedSum(values, from, middle).constAdd(balancedSum(values, middle, to)).shared()
    }
  }

private fun lengthPrefixes(lengths: List<RemoteFloat>): List<RemoteFloat> =
  lengths.runningFold(0f.rf) { prefix, length -> prefix.constAdd(length).shared() }

private data class TrimPoint(val x: RemoteFloat, val y: RemoteFloat) {
  operator fun plus(other: TrimPoint) = TrimPoint(x.constAdd(other.x), y.constAdd(other.y))

  operator fun minus(other: TrimPoint) = TrimPoint(x.constSub(other.x), y.constSub(other.y))

  operator fun times(f: RemoteFloat) = TrimPoint(x.constMul(f), y.constMul(f))

  fun shared() = TrimPoint(x.shared(), y.shared())

  fun length() = x.constMul(x).constAdd(y.constMul(y)).constSqrt().shared()
...
private class MeasuredCubic(val curve: TrimCubic, val straight: Boolean, present: RemoteFloat) {
  ...
  val length = geometricLength.constMul(present).shared()

  fun parameter(distance: RemoteFloat): RemoteFloat {
    if (straight)
      return straightParameter(
        clamp(distance / constMax(geometricLength, 1e-9f.rf), 0f.rf, 1f.rf).shared()
      )
    return balancedSum(
      (0 until table.lastIndex).map { j ->
        val denom = constMax(table[j + 1].constSub(table[j]), 1e-9f.rf)
        val part = clamp(distance.constSub(table[j]) / denom, 0f.rf, 1f.rf)
  ...
  fun cut(fromDistance: RemoteFloat, toDistance: RemoteFloat): TrimCubic {
    if (straight) {
      val denom = constMax(geometricLength, 1e-9f.rf)
      val f0 = clamp(fromDistance / denom, 0f.rf, 1f.rf).shared()
      val f1 = clamp(toDistance / denom, 0f.rf, 1f.rf).shared()
      val s = mix(curve.a, curve.d, f0)
      val e = mix(curve.a, curve.d, f1)
      return TrimCubic(s, s, e, e)
    }
    return curve.cut(parameter(fromDistance), parameter(toDistance))
  }
}
