...
internal fun sliceCubic(
  points: List<RemoteFloat>,
  a: RemoteFloat,
  b: RemoteFloat,
): List<RemoteFloat> {
  ...
  val cubic =
    OffsetCubic(
      CurvePoint(points[0], points[1]),
      CurvePoint(points[2], points[3]),
      CurvePoint(points[4], points[5]),
      CurvePoint(points[6], points[7]),
    )
  val sliced = cubic.slice(a, b)
  return listOf(sliced.p0, sliced.p1, sliced.p2, sliced.p3).flatMap { it.values() }
}
...
private fun OffsetCubic.breakpoints(): List<RemoteFloat> {
  ...
  return listOf(0f.rf, min(low, high), max(low, high), 1f.rf)
}
...
/**
 * Returns true when [OffsetPath] is applied to a variable-topology contour or live-animated curve
 * whose topology/self-intersection count can change across playback frames.
 */
internal fun RemoteBezierValue.requiresOffsetTopologyDegradation(): Boolean {
  val topo = topology ?: return false
  return topo.vertices.any { it.constantValueOrNull == null } ||
    topo.segments.any { it.constantValueOrNull == null }
}

/**
 * Fails fast when [OffsetPath] is applied to a variable-topology contour or live-animated curve
 * whose topology/self-intersection count can change across playback frames.
 */
internal fun RemoteBezierValue.requireSupportedOffsetTopology(
  hasCurves: Boolean,
  amount: RemoteFloat,
  miterLimit: RemoteFloat,
) {
  val hasAnimatedCurveOffset =
    hasCurves &&
      (amount.constantValueOrNull == null ||
        miterLimit.constantValueOrNull == null ||
        (vertices + inTangents + outTangents).flatten().any { it.constantValueOrNull == null })
  require(!requiresOffsetTopologyDegradation() && !hasAnimatedCurveOffset) {
...
internal fun offsetCurves(
  ...
  if (strictOffsetTopology) {
    input.requireSupportedOffsetTopology(hasCurves = true, amount = amount, miterLimit = miterLimit)
  }
  val degrade = input.requiresOffsetTopologyDegradation()
  ...
  var groups = curves.mapIndexed { index, raw ->
    ...
    if (degrade) {
      OffsetPieces(
        listOf(if (index < source.size) raw else collapsed(source.last().p3)),
        listOf(1f.rf),
        mutableListOf(curve.offset(amount, fallback, 0f.rf, 0f.rf).shared()),
      )
    } else {
  ...
  val pruned =
    if (degrade) {
      List(groups.size) { 0f.rf }
    } else if (path.topology != null) {
  ...
  for ((index, raw) in curves.withIndex()) {
    ...
    val fallback = (curve.p3 - curve.p0).unit(CurvePoint(1f.rf, 0f.rf))
    for (piece in groups[index].pieces.indices) {
